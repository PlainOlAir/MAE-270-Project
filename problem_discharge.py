...
def smooth_positive(z, eps=1e-3, sqrt_fn=csdl.sqrt):
    # C1-smooth approximation to max(z, 0), avoids exp overflow.
...
def get_problem_params():
    ...
    return dict(
        ...
        m_payload=30.0,
        sigma_panel=2,          # https://www.spectrolab.com/DataSheets/Panel/panels.pdf
        a_struct=1.2,
        b_struct=0.04,
        WS_max=250.0,
        b_max=40.0,
...
# ---------- Numpy smooth ----------
def smooth_positive_np(z, eps=1e-3):
    return smooth_positive(z, eps=eps, sqrt_fn=np.sqrt)
...
def derive_outputs_from_design(x, params):
    ...
    P_req, P_solar, T_endurance_s = add_power_energy_endurance(
